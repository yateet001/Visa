#!/usr/bin/env python3
import argparse, json, os, sys, tempfile, zipfile, requests, time, subprocess, shutil
from auth import get_token
from deploy import wait_for_dataset, get_datasources, build_update_requests, update_datasources
...
def build_pbix_with_pbitools(input_path):
    """Build a PBIX from a PBIP project using pbi-tools if available"""
    project_dir = input_path
    if input_path.endswith('.pbip'):
        project_dir = os.path.dirname(input_path)
    pbi_tools_path = shutil.which('pbi-tools')
    if not pbi_tools_path:
        raise FileNotFoundError("pbi-tools not found in PATH")
    temp_pbix = tempfile.NamedTemporaryFile(suffix='.pbix', delete=False)
    temp_pbix.close()
    cmd = [pbi_tools_path, 'build', project_dir, '--out', temp_pbix.name]
    print(f"🛠️ Using pbi-tools to build PBIX: {' '.join(cmd)}")
    completed = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, timeout=600)
    print(completed.stdout)
    if completed.returncode != 0 or not os.path.exists(temp_pbix.name):
        if os.path.exists(temp_pbix.name):
            os.unlink(temp_pbix.name)
        raise RuntimeError(f"pbi-tools build failed with code {completed.returncode}")
    size_bytes = os.path.getsize(temp_pbix.name)
    print(f"✅ pbi-tools built PBIX: {temp_pbix.name}")
    print(f"📏 File size: {size_bytes} bytes")
    if size_bytes < 1024 * 50:
        print("⚠️ Built PBIX seems unusually small; import may fail")
    return temp_pbix.name
...
def main():
    ...
    p.add_argument('--pbix', required=True, help='Path to PBIX file or PBIP project (folder or .pbip)')
    args = p.parse_args()
    ...
    try:
        # Determine input type and produce PBIX accordingly
        input_path = args.pbix
        if not os.path.exists(input_path):
            raise FileNotFoundError(f"Input path not found: {input_path}")
        if input_path.lower().endswith('.pbix') and os.path.isfile(input_path):
            print('\n📦 Using provided PBIX file...')
            pbix_file = input_path
        else:
            # Prefer pbi-tools build when available
            try:
                print('\n📦 Building PBIX from PBIP using pbi-tools...')
                pbix_file = build_pbix_with_pbitools(input_path)
            except Exception as build_err:
                print(f"⚠️ pbi-tools build unavailable or failed: {build_err}")
                print('🔁 Falling back to internal PBIX packer (may be limited)...')
                pbix_file = create_proper_pbix_from_pbip(input_path)
        
        # Early sanity check: tiny files are likely invalid
        try:
            size_bytes = os.path.getsize(pbix_file)
            if size_bytes < 50 * 1024:
                raise Exception(f"Generated PBIX too small ({size_bytes} bytes). Ensure pbi-tools is installed and PBIP is valid.")
        ...
        # After successful import, perform connection switching for the dataset
        try:
            print('\n🔗 Updating dataset connections...')
            dataset_id = wait_for_dataset(token, workspace_id, report_name, timeout=180)
            datasources = get_datasources(token, workspace_id, dataset_id)
            print(f"🔎 Found {len(datasources)} datasources")
            updates = build_update_requests(
                datasources,
                cfg['warehouseConnection'],
                cfg['warehouseName']
            )
            if updates:
                _ = update_datasources(token, workspace_id, dataset_id, updates)
                print('✅ Connection switching completed')
            else:
                print('ℹ️ No datasource updates required')
        except Exception as e:
            print(f"⚠️ Connection update skipped/failed: {e}")
        ...
        try:
            if pbix_file and os.path.exists(pbix_file):
                # Do not delete if the input was an existing pbix path
                if not (os.path.exists(args.pbix) and os.path.isfile(args.pbix) and os.path.samefile(pbix_file, args.pbix)):
                    os.unlink(pbix_file)
